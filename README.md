...
一个基于 Chrome Manifest V3 的浏览器扩展，用来把 `mp.weixin.qq.com` 的公众号文章、`zhuanlan.zhihu.com` 的知乎专栏文章、`juejin.cn/post/*` 的掘金文章与 `blog.csdn.net/*/article/details/*` 的 CSDN 文章下载为：
...
- 支持 `https://mp.weixin.qq.com/*`、`https://zhuanlan.zhihu.com/p/*`、`https://juejin.cn/post/*` 与 `https://blog.csdn.net/*/article/details/*` 文章页
...
4. 选择“加载已解压的扩展程序”
5. 选择本项目的 `dist/` 目录
6. 打开一篇支持的网站文章（微信公众号、知乎专栏、掘金或 CSDN）
...
对应的离线 fixture 保存在 `tests/fixtures/weixin-article.html`、`tests/fixtures/zhihu-article.html`、`tests/fixtures/juejin-article.html` 与 `tests/fixtures/csdn-article.html`。
